...
The eighteen gaps the first Final-designs pass recorded here (a region control
in the nav, kicker links and action rows on the show pages, a date toggle
beside the day strip, class hooks on the filters, a partner search field, the
partner event browser's header, repeating select and "show more days", a CSV
link on the events index, news pagination in both directions, article image
credit, pull quote and previous/next, ordinal dates, and a theme-chosen
default events period) were built in core the same day, on the branch
`td/final-design-slots` (PlaceCal PR to follow), and this theme consumes them
through `lib/transdimension/engine.rb` and the route partials. Items 1 to 5
above still stand.

What the design still asks for that core does not have:

### 6. Visit website / Email / Call buttons on the partner page

- **Core**: `app/components/contact_details.rb` renders the channels as one
  list of icon-and-link rows (now with a class per channel).
- **Element**: `reference/ui_kit/Final.jsx.txt` `PartnerF` draws the same three
  contacts a second time as buttons under the GET IN TOUCH column.
- **Slot**: a `buttons: true` variant on `Components::ContactDetails`, or the
  theme rendering its own partner page (out of contract today).
- **Consequence today**: the link list only.

### 7. A "Pick a date" toggle on the partner page's event browser

- **Core**: `app/views/partners/show.rb` `render_events_header` renders the
  heading, the count and the repeating select; the date picker belongs to
  `Components::EventFilter`, which the flat browser no longer shows.
- **Slot**: render the date toggle from `EventFilter` in the header row when
  the theme's `event_filter_style` includes the date picker.
- **Consequence today**: the header row has the Show select only.

### 8. "View more news" on the news index

- **Element**: the design's middle button. The prototype gives it no behaviour
  beyond returning to page one, so it is left out rather than invented.
